...
export async function handleList(ctx: BotContext) {
  try {
    ...
    const activeInChat = tracked.filter((t) => t.chat_id === chatId);
    if (activeInChat.length > 0) {
      const lines = activeInChat.map((t) => {
        const msgLink = buildMessageLink(chatId, t.message_id);
    ...
    const pendingInChat = pendingActions.filter((p) => p.chat_id === chatId);
    const validPending: string[] = [];

    for (const pending of pendingInChat) {
      ...
      const msgLink = job ? buildMessageLink(chatId, job.message_id) : "";
